...
async function organizeTabsByTLD(tldToTabs) {
    try {
        ...
        for (const [tld, tabs] of Object.entries(tldToTabs)) {
            ...
            // Find an existing window with tabs from this TLD
            for (const [windowId, window] of existingWindows) {
                if (window.tabs.some(t => new URL(t.url).hostname.split('.').slice(-2).join('.') === tld)) {
            ...
            if (!targetWindowId) {
                ...
                // Remove the first tab from the array as it's already in the new window
                tabs.shift();
            }
            
            // Move remaining tabs to the target window
